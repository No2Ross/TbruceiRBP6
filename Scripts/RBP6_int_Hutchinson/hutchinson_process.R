...
#Load Hutchinson data
load("/data/Ross/lucy_rbp6_analysis/paper/integration_analysis/objects/T1_3")

T1_3 <- UpdateSeuratObject(T1_3)

T1_3$group <- "inVivo"
T1_3$celltype <- T1_3@active.ident
T1_3@reductions$pca@assay.used <- "RNA"
T1_3@reductions$umap@assay.used <- "RNA"
T1_3@assays$SCT <- NULL
T1_3@assays$integrated <- NULL


#Need to SCRAN normalise the data
hutchinson_split <- SplitObject(T1_3, "orig.ident")

hutchinson_obj <- hutchinson_split$Lib1

exp_mtx <- hutchinson_obj@assays$RNA@counts
metaData <- hutchinson_obj@meta.data

hutchinson_obj <- CreateSeuratObject(exp_mtx, meta.data = metaData)

Idents(hutchinson_obj)<- "celltype"

hutchinson_obj <- NormalizeData(hutchinson_obj)
...
ribosomal_genes <- intersect(ribosomal_genes, row.names(hutchinson_obj))

#Find ribosomal proteins that are expressed in more than 10% of cells
exp_mtx <- hutchinson_obj@assays$RNA@layers$counts
row.names(exp_mtx) <- row.names(hutchinson_obj)

riboProtein_percent <- rowSums(exp_mtx[ ribosomal_genes,] > 0) / dim(hutchinson_obj)[2]

riboProtein_percent <- riboProtein_percent[which(riboProtein_percent >= 0.1)]

hutchinson_obj <- MetaFeature(hutchinson_obj, features = names(riboProtein_percent), meta.name = "RiboProtein")
...
cellcyclemarkers <- intersect(cellcyclemarkers, row.names(hutchinson_obj))

## calculate the percentage of cells expressing each marker genes
cell_prct <- PrctCellExpringGene(hutchinson_obj, genes = cellcyclemarkers, group.by = "all")
...
hutchinson_obj <- MetaFeature(hutchinson_obj, features = s.genes, meta.name = "S.aggregate")
hutchinson_obj <- MetaFeature(hutchinson_obj, features = g2m.genes, meta.name = "G2M.aggregate")
hutchinson_obj <- MetaFeature(hutchinson_obj, features = g1e.genes, meta.name = "G1e.aggregate")
hutchinson_obj <- MetaFeature(hutchinson_obj, features = g1l.genes, meta.name = "G1l.aggregate")


# Create and dataframe with the expression score of each cell and each phase
dfLog <- data.frame(hutchinson_obj@meta.data[["S.aggregate"]], 
                    hutchinson_obj@meta.data[["G2M.aggregate"]], 
                    hutchinson_obj@meta.data[["G1e.aggregate"]],
                    hutchinson_obj@meta.data[["G1l.aggregate"]])

colnames(dfLog) <- c("S", "G2M", "G1e", "G1l")
rownames(dfLog) <- colnames(hutchinson_obj)
...
pdf("RBP6_analysis/plots/G1e_histogram.pdf")
p1
dev.off()
...
pdf("RBP6_analysis/plots/G1l_histogram.pdf")
p2
dev.off()
...
pdf("RBP6_analysis/plots/G2M_histogram.pdf")
p4
dev.off()
...
dfLog$Cluster <- hutchinson_obj@active.ident

hutchinson_obj$Phase <- dfLog$Phase


saveRDS(hutchinson_obj,"/data/Ross/lucy_rbp6_analysis/paper/integration_analysis/objects/seb_lib1_processed.rds")
